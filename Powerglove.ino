...
float gyroBias[3] = {0, 0, 0}, accelBias[3] = {0, 0, 0}, magBias[3] = {0, 0, 0}, magScale[3]  = {0, 0, 0}, fingBias[4] = {0, 0, 0, 0}, fingScale[4] = {1, 1, 1, 1};     // Bias corrections for gyro and accelerometer

#define OFFSET_ACCEL 0
#define OFFSET_GYRO 6 * sizeof(uint8_t)
#define OFFSET_MAG OFFSET_GYRO + 6 * sizeof(uint8_t)
#define OFFSET_FLEX OFFSET_MAG + 6 *sizeof(float)

void writeAccel(uint8_t * data){
  EEPROM.put(OFFSET_ACCEL, data[0]); 
  EEPROM.put(OFFSET_ACCEL + 1, data[1]); 
  EEPROM.put(OFFSET_ACCEL + 2, data[2]); 
  EEPROM.put(OFFSET_ACCEL + 3, data[3]); 
  EEPROM.put(OFFSET_ACCEL + 4, data[4]); 
  EEPROM.put(OFFSET_ACCEL + 5, data[5]);
  Serial.print("Save Accel 1-3"); Serial.print( data[0] ); Serial.print(" ,  ");Serial.print( data[1] ); Serial.print(" ,  ");Serial.print( data[2] ); Serial.println(" ");
  Serial.print("Save Accel 4-6 "); Serial.print( data[3] ); Serial.print(" ,  ");Serial.print( data[4] ); Serial.print(" ,  ");Serial.print( data[5] ); Serial.println(" "); 
}

void writeGyro(uint8_t * data){
  EEPROM.put(OFFSET_GYRO, data[0]); 
  EEPROM.put(OFFSET_GYRO + 1, data[1]); 
  EEPROM.put(OFFSET_GYRO + 2, data[2]); 
  EEPROM.put(OFFSET_GYRO + 3, data[3]); 
  EEPROM.put(OFFSET_GYRO + 4, data[4]); 
  EEPROM.put(OFFSET_GYRO + 5, data[5]);
  Serial.print("Save Gyro 1-3 "); Serial.print( data[0] ); Serial.print(" ,  ");Serial.print( data[1] ); Serial.print(" ,  ");Serial.print( data[2] ); Serial.println(" ");
  Serial.print("Save Gyro 4-6 "); Serial.print( data[3] ); Serial.print(" ,  ");Serial.print( data[4] ); Serial.print(" ,  ");Serial.print( data[5] ); Serial.println(" ");
}

void writeMag(float* bias, float* scale){
  EEPROM.put(OFFSET_MAG, bias[0]); 
  EEPROM.put(OFFSET_MAG + 1 * sizeof(float), bias[1]);
  EEPROM.put(OFFSET_MAG + 2 * sizeof(float), bias[2]);
  EEPROM.put(OFFSET_MAG + 3 * sizeof(float), scale[0]);
  EEPROM.put(OFFSET_MAG + 4 * sizeof(float), scale[1]);
  EEPROM.put(OFFSET_MAG + 5 * sizeof(float), scale[2]);
  Serial.print("Save Mag Bias "); Serial.print( bias[0] ); Serial.print(" ,  ");Serial.print( bias[1] ); Serial.print(" ,  ");Serial.print( bias[2] ); Serial.println(" ");
  Serial.print("Save Mag Scale "); Serial.print( scale[0] ); Serial.print(" ,  ");Serial.print( scale[1] ); Serial.print(" ,  ");Serial.print( scale[2] ); Serial.println(" ");
}


void writeFlex(float* bias, float * scale){
  EEPROM.put(OFFSET_FLEX, bias[0]); 
  EEPROM.put(OFFSET_FLEX + 1 * sizeof(float), bias[1]);
  EEPROM.put(OFFSET_FLEX + 2 * sizeof(float), bias[2]);
  EEPROM.put(OFFSET_FLEX + 3 * sizeof(float), bias[3]);
  EEPROM.put(OFFSET_FLEX + 4 * sizeof(float), scale[0]);
  EEPROM.put(OFFSET_FLEX + 5 * sizeof(float), scale[1]);
  EEPROM.put(OFFSET_FLEX + 6 * sizeof(float), scale[2]);
  EEPROM.put(OFFSET_FLEX + 7 * sizeof(float), scale[3]);
}

void loadAccelGyro(){
  uint8_t data[6]; // data array to hold accelerometer and gyro x, y, z, data

 EEPROM.get(OFFSET_ACCEL, data); 
  ...
  Serial.print("Load Accel 1-3"); Serial.print( data[0] ); Serial.print(" ,  ");Serial.print( data[1] ); Serial.print(" ,  ");Serial.print( data[2] ); Serial.println(" ");
  Serial.print("Load Accel 4-6 "); Serial.print( data[3] ); Serial.print(" ,  ");Serial.print( data[4] ); Serial.print(" ,  ");Serial.print( data[5] ); Serial.println(" ");
  
 EEPROM.get(OFFSET_GYRO, data); 
  ...
  Serial.print("Load Gyro 1-3 "); Serial.print( data[0] ); Serial.print(" ,  ");Serial.print( data[1] ); Serial.print(" ,  ");Serial.print( data[2] ); Serial.println(" ");
  Serial.print("Load Gyro 4-6 "); Serial.print( data[3] ); Serial.print(" ,  ");Serial.print( data[4] ); Serial.print(" ,  ");Serial.print( data[5] ); Serial.println(" ");
  
}


void loadMag(){
 EEPROM.get(OFFSET_MAG, magBias); 
 EEPROM.get(OFFSET_MAG + 3 * sizeof(float), magScale);
  Serial.print("Load Mag Bias "); Serial.print( magBias[0] ); Serial.print(" ,  ");Serial.print( magBias[1] ); Serial.print(" ,  ");Serial.print( magBias[2] ); Serial.println(" ");
  Serial.print("Load Mag Scale "); Serial.print( magScale[0] ); Serial.print(" ,  ");Serial.print( magScale[1] ); Serial.print(" ,  ");Serial.print( magScale[2] ); Serial.println(" ");
}


void loadFlex(){
 EEPROM.get(OFFSET_FLEX, fingBias); 
 EEPROM.get(OFFSET_FLEX + 4 * sizeof(float), fingScale); 
}
...
//-----------------------(Buttons)--------------------------------------
int u = 1;
int k = 1;// integer used in scanning the array designating column number
//2-dimensional array for asigning the buttons and there high and low values
int Button1[7][3] = {{0, 0, 130}, // NONE
                    {1, 810, 850}, // A
                    {2, 730, 780}, // B
                    {3, 510, 610}, // Start
                    {4, 280, 320}, // Select
                    ...
                    //{11, 777, 786}, // B + Select
                    //{12, 770, 776}, // B + Center
                    //{13, 660, 670}, // Start + Select
                    //{14, 620, 635}, // Start + Center
                    //{15, 400, 401}}; // Select + Center
int analogpin7 = 7; // analog pin to read the buttons
int label1 = 0;  // for reporting the button label
int counter1 = 0; // how many times we have seen new value
long time = 0;  // the last time the output pin was sampled
int debounce_count1 = 20; // number of millis/samples to consider before declaring a debounced input
int current_state1 = 0;  // the debounced input value
int ButtonVal1;
int last_buttonVal1;

int Button2[5][3] = {{0, 0, 200}, // NONE
                    {1, 700, 830}, // Down
                    {2, 540, 620}, //Left
                    {3, 420, 460}, // Up
                    {4, 280, 340}}; // Right

int label2 = 0;  // for reporting the button label
int counter2 = 0; // how many times we have seen new value
int debounce_count2 = 20; // number of millis/samples to consider before declaring a debounced input
int current_state2 = 0;  // the debounced input value
int ButtonVal2;
int last_buttonVal2; 
int analogpin6 = 6;


int modus = 1;  //Chooses which Moodus should be used to define the Serial Output. Can be 0, 1 or 2
                 //1 = Rawdata , 2 =  Madqwick Quaternion , 3 = Pitch, yaw, roll

//------------------------------(Fingers)------------------------------------

const int FLEX_PIN0 = A0; //defines the Inputpins of the Flexsensors
...
float flexADC0;  //defines the raw Values for the Flexsensors
float flexADC1;
float flexADC2;
float flexADC3;
...
void setup()
{
  ...
  //Input for the Button with Pullop
  ...
  if (c == 0x68) // WHO_AM_I should always be 0x68
  {
    ...
    //magcalMPU9150(magBias, magScale);
    Serial.println("AK8975A mag biases (mG)"); Serial.println(magBias[0]); Serial.println(magBias[1]); Serial.println(magBias[2]); 
    Serial.println("AK8975A mag scale (mG)"); Serial.println(magScale[0]); Serial.println(magScale[1]); Serial.println(magScale[2]); 
...
// If we have gone on to the next millisecond
 if (millis() != time)
 {
   ...
   ButtonVal1 = ButtonCheck1();
   if((ButtonVal1 == current_state1 || ButtonVal1 != last_buttonVal1)&& counter1 >0)
   { 
     counter1--;
   }
   if(ButtonVal1 != current_state1 && ButtonVal1 == last_buttonVal1)
   {
     counter1++;
   }
   // If ButtonVal has shown the same value for long enough let's switch it
   if (counter1 >= debounce_count1)
   {
     counter1 = 0;
     current_state1 = ButtonVal1;
 ...
 {
   // check analog pin for the button value and save it to ButtonVal
   ButtonVal2 = ButtonCheck2();
   if((ButtonVal2 == current_state2 || ButtonVal2 != last_buttonVal2) && counter2 >0)
   { 
     counter2--;
   }
   if(ButtonVal2 != current_state2 && ButtonVal2 == last_buttonVal2)
   {
     counter2++;
   }
   // If ButtonVal has shown the same value for long enough let's switch it
   if (counter2 >= debounce_count2)
   {
     counter2 = 0;
     current_state2 = ButtonVal2;
     //Checks which button or button combo has been pressed
     if (ButtonVal2 >= 0)
     {
       label2 = ButtonVal2;
 ...
 //----------------------------------------(Buttons end)-------------------------------------------
  ...
  if (readByte(MPU9150_ADDRESS, INT_STATUS) & 0x01) {  // On interrupt, check if data ready interrupt
    ...
    flexADC0 = analogRead(FLEX_PIN0) - fingBias[0];
    flexADC1 = analogRead(FLEX_PIN1) - fingBias[1];
    flexADC2 = analogRead(FLEX_PIN2) - fingBias[2];
    flexADC3 = analogRead(FLEX_PIN3) - fingBias[3];
    flexADC0 *= fingScale[0];
    flexADC1 *= fingScale[1];
    flexADC2 *= fingScale[2];
    flexADC3 *= fingScale[3];

    if(flexADC0 > 1.0){flexADC0 = 1.0;}
      else{
        if(flexADC0 < 0.0){flexADC0 = 0.0;}
      }

     if(flexADC1 > 1.0){flexADC1 = 1.0;}
      else{
        if(flexADC1 < 0.0){flexADC1 = 0.0;}
      }

     if(flexADC2 > 1.0){flexADC2 = 1.0;}
      else{
        if(flexADC2 < 0.0){flexADC2 = 0.0;}
      }

     if(flexADC3 > 1.0){flexADC3 = 1.0;}
      else{
        if(flexADC3 < 0.0){flexADC3 = 0.0;}
      }
    ...
    if (delt_t > 500) { // update LCD once per half-second independent of read rate
      ...
      if (SerialDebug) { 
        // Reads out the Raw data in form :
        // ax, ay, az
        // gx, gy, gz
        // mx , my , mz
        ...
        Serial.print(flexADC0); Serial.println(","); 
        ...
        //Serial.print("gyroBias:"); Serial.print(gyroBias[0]); Serial.print(gyroBias[1]); Serial.print(gyroBias[2]); Serial.println(",");
        //Serial.print("accelBias:"); Serial.print(accelBias[0]); Serial.print(accelBias[1]); Serial.print(accelBias[2]); Serial.println(",");
        //Serial.print("magBias:"); Serial.print(magBias[0]); Serial.print(magBias[1]); Serial.print(magBias[2]); Serial.println(",");
        //Serial.print("magScale:"); Serial.print(magScale[0]); Serial.print(magScale[1]); Serial.print(magScale[2]); Serial.println(",");
        //Serial.print("average rate = "); Serial.print(1.0f / deltat, 2); Serial.println(" Hz");
        //Serial.print(ButtonVal1); Serial.println("");
        //Serial.print(magCalibration[0]); Serial.print(","); Serial.print(magCalibration[1]); Serial.print(",,");
    ...
    pitch *= 180.0f / PI;
    yaw   *= 180.0f / PI;
    yaw   -= 14.5; // Declination at Danville, California is 13 degrees 48 minutes and 47 seconds on 2014-04-04
    roll  *= 180.0f / PI;
...
void initAK8975A(float * destination)
{
  ...
  //writeByte(AK8975A_ADDRESS, AK8975A_CNTL, 0x00); // Power down magnetometer  
  //delay(10);
  // Configure the magnetometer for continuous read and highest resolution
  // set Mscale bit 4 to 1 (0) to enable 16 (14) bit resolution in CNTL register,
  // and enable continuous mode data acquisition Mmode (bits [3:0]), 0010 for 8 Hz and 0110 for 100 Hz sample rates
  //writeByte(AK8975A_ADDRESS, AK8975A_CNTL, Mscale << 4 | Mmode); // Set magnetometer data resolution and sample ODR
  //delay(10);
}
...
void calibrateMPU9150(float * dest1, float * dest2)
{
  ...
  Serial.print("KALIBRIERT") + Serial.println("");
...
void magcalMPU9150(float * dest1, float * dest2) 
{
  ...
  //delay(4000);
    ...
    if(Mmode == 0x06) sample_count = 999;  // at 100 Hz ODR, new mag data is available every 10 ms
   for(ii = 0; ii < sample_count; ii++) {
    if(ii%100 == 0)Serial.println((sample_count - ii)/100);
    lcd.setCursor(12, 1);
    lcd.print(sample_count - ii);
    lcd.print("  ");

    readMagData(mag_temp);  // Read the mag data   
    //Serial.print(mag_temp[0]) ; Serial.print(",") ; Serial.print(mag_temp[1]) ; Serial.print(",") ; Serial.print(mag_temp[2]) ; Serial.println("");  
...
//    Serial.println("mag z min/max:"); Serial.println(mag_max[2]); Serial.println(mag_min[2]);
   ...
   writeMag(dest1,dest2);
   Serial.println("Mag Calibration done!");
}



void calfingers(float * dest1, float * dest2) 
{
  int pp = 0, sample_count = 500;
  int fing_bias[4] = {0, 0, 0, 0}, fing_scale[4] = {0, 0, 0, 0};
  int fing_max[4] = {-1000, -1000, -1000, -1000}, fing_min[4] = {1000, 1000, 1000, 1000}, fing_temp[4] = {0, 0, 0, 0};

  Serial.println("Finger Calibration: Bend and stretch fingers until done!");
  //delay(4000);
  
   for(pp = 0; pp < sample_count; pp++) {
    if(pp%100 == 0)Serial.println(sample_count - pp);
    fing_temp[0] = analogRead(FLEX_PIN0);
    fing_temp[1] = analogRead(FLEX_PIN1);
    fing_temp[2] = analogRead(FLEX_PIN2);
    fing_temp[3] = analogRead(FLEX_PIN3);  
     
    //Serial.print(mag_temp[0]) ; Serial.print(",") ; Serial.print(mag_temp[1]) ; Serial.print(",") ; Serial.print(mag_temp[2]) ; Serial.println("");  
      for (int jj = 0; jj < 4; jj++) {
      if(fing_temp[jj] > fing_max[jj]) fing_max[jj] = fing_temp[jj];
      if(fing_temp[jj] < fing_min[jj]) fing_min[jj] = fing_temp[jj];
      }
    ...
    fing_scale[0]  = (fing_max[0] - fing_min[0]);  // get average x axis max chord length in counts
    fing_scale[1]  = (fing_max[1] - fing_min[1]);
    fing_scale[2]  = (fing_max[2] - fing_min[2]);
    fing_scale[3]  = (fing_max[3] - fing_min[3]);


    dest2[0] = (float) 1.2 / fing_scale[0];
    dest2[1] = (float) 1.2 / fing_scale[1];
    dest2[2] = (float) 1.2 / fing_scale[2];
    dest2[3] = (float) 1.2 / fing_scale[3];

    fing_bias[0]  = fing_min[0];  // get average x mag bias in counts
    fing_bias[1]  = fing_min[1];  // get average y mag bias in counts
    fing_bias[2]  = fing_min[2]; 
    fing_bias[3]  = fing_min[3]; // get average z mag bias in counts

    
    dest1[0] = (float) fing_bias[0] + 0.1 / dest2[0] ;  // save mag biases in G for main program
    dest1[1] = (float) fing_bias[1] + 0.1 / dest2[1] ;   
    dest1[2] = (float) fing_bias[2] + 0.1 / dest2[2] ;
    dest1[3] = (float) fing_bias[3] + 0.1 / dest2[3] ;
    writeFlex(dest1, dest2);
  Serial.println("Finger Calibration done!");
...
int ButtonCheck2()
{
 int val = analogRead(analogpin6); 
 // loop for scanning the button array.
 for(int i = 0; i <= 5; i++)
 {
   // checks the ButtonVal against the high and low vales in the array
   if(val >= Button2[i][k] && val <= Button2[i][k+1])
   {
     // stores the button number to a variable
     return Button2[i][0];
   }
...
void Buttons(){
//-----------------------------------------(Display/ Kalibration Menu)-----------------------------------------------


 
 // button = digitalRead(4);
  
 if (modus == 1) {                       //Accelometer and Gyroscope Calibration
    lcd.print("< Acc-Gyro-Cal >");
    lcd.setCursor(0, 1);
    lcd.print("A=Yes , Slt=Next");
   ...
   if(modus == 2) {                             // Magnetometer Calibration
      lcd.setCursor(0, 0); 
      lcd.print("< Magneto-Cal >");
      lcd.setCursor(0, 1);
      lcd.print("A=Yes , Slt=Next");
...
if(modus == 3) {                              //Finger Calibration
      
      lcd.setCursor(0, 0); 
      lcd.print("< Finger-Cal >");
      lcd.setCursor(0, 1);
      lcd.print("A=Yes , Slt=Next");
      ...
      if(label1 == 1){
        ...
        lcd.clear();
        lcd.setCursor(0, 0);
        lcd.print("Stretch Fingers");
        lcd.setCursor(0, 1);
        lcd.print("+ close til done");
        calfingers(fingBias, fingScale);
        //Serial.println("FingerBias"); Serial.println(fingBias[0]); Serial.println(fingBias[1]); Serial.println(fingBias[2]); Serial.println(fingBias[3]); 
        //Serial.println("FingerScale"); Serial.println(fingScale[0]); Serial.println(fingScale[1]); Serial.println(fingScale[2]); Serial.println(fingScale[3]);
        lcd.clear();
        lcd.setCursor(6, 0);
        lcd.print("Done");
        delay(1000);
